...
import "./Hauptspeisen.css";
import { useParams } from 'react-router-dom';

// Funktionskomponente für Hauptspeisen
export default function Hauptspeisen() {
    ...
    function suchen(begriff) {
        readJSONFromServer(
            `http://localhost:8899/hauptspeise/finden/${begriff}`,
            (respond) => {
    ...
    function zumRezept(nummer) {
        ...
        readJSONFromServer(
            `http://localhost:8899/hauptspeise/anzeigen/${nummer}`,
            (respond) => {
                const daten = respond.map((row) => {
    ...
    function alleRezepte() {
        ...
        readJSONFromServer(`http://localhost:8899/hauptspeise/alleAnzeigen`, (respond) => {
            const daten = respond.map((row) => (
