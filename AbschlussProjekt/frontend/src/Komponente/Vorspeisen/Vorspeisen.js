...
import "./Vorspeisen.css";
import { useParams } from 'react-router-dom';

// Funktionskomponente für Vorspeisen
export default function Vorspeisen() {
    ...
    function suchen(begriff) {
        readJSONFromServer(
            `http://localhost:8899/vorspeise/finden/${begriff}`,
            (respond) => {
                // Serverantwort verarbeiten und Zustand aktualisieren
    ...
    function sammleDaten() {
        ...
        const zutatenJSON = JSON.stringify(zutatenArray.map(zutat => {
            ...
            // Entfernen Sie das wiederholte 'zutat:' Präfix, falls vorhanden
    ...
    function speichern() {
        ...
        fetch(`http://localhost:8899/rezept/edit/${selectedCategory}/${rezeptNr}`, {
            ...
            .then((data) => {
                ...
                // Bearbeitungsmodus für das aktuelle Rezept deaktivieren
                setBearbeitungsModus((prevModus) => ({
                    ...prevModus,
                    [rezeptNr]: false,
                }));
            ...
            .catch((error) => {
                ...
                // Bearbeitungsmodus auch im Fehlerfall deaktivieren
                setBearbeitungsModus((prevModus) => ({
                    ...prevModus,
                    [rezeptNr]: false,
                }));
    ...
    function zumRezept(nummer) {
        // Details zu einem Rezept vom Server abrufen
        const inpSuche = document.getElementById("suchInput");
        setAktuellerRezeptNr(nummer);

        readJSONFromServer(
            `http://localhost:8899/vorspeise/anzeigen/${nummer}`,
            (respond) => {
                const daten = respond.map((row) => {
                        ...
                        <li key={row.RezeptNr} className="bearbeiten">
                            {bearbeitungsModus[row.RezeptNr] ? (
                                <>
                                    ...
                                    >
                                        <option value={"Vorspeise"} selected>Vorspeise</option>
    ...
    function alleRezepte() {
        // Alle Rezepte vom Server abrufen
        readJSONFromServer(`http://localhost:8899/vorspeise/alleAnzeigen`, (respond) => {
            const daten = respond.map((row) => (
